...
description: 'Hat tip to profiling and flamegraphs'
publishedAt: '2024-11-12 00:00:00.000'
author: nholden
---
...
- An [Axiom monitor](https://axiom.co/docs/monitor-data/monitors) sent us a “high API request queue time” message via [our Campsite integration](https://developers.campsite.com/). Web server processes were fully utilized, and requests were forced to wait.
- [Fly.io metrics](https://fly.io/docs/monitoring/metrics/) showed HTTP response times were significantly elevated, but didn’t show an increase in traffic.
- [PlanetScale insights](https://planetscale.com/docs/concepts/query-insights) didn’t show any increase in database query latency.
...
To figure out why the new code was slow, after the revert, we profiled it in development. We use [rack-mini-profiler](https://github.com/MiniProfiler/rack-mini-profiler?tab=readme-ov-file) in development and production to profile Ruby code.

rack-mini-profiler works best out-of-the-box for traditional Rails applications that render HTML. Our Rails app primarily serves as an API for our client Next.js application, so it mostly renders JSON, but we still find rack-mini-profiler valuable. One way is with rack-mini-profiler’s [built-in route](https://github.com/MiniProfiler/rack-mini-profiler?tab=readme-ov-file#using-miniprofilers-built-in-route-for-apps-without-html-responses) that serves a blank page with a speed badge.

To debug this performance regression we started with [flamegraphs](https://github.com/MiniProfiler/rack-mini-profiler?tab=readme-ov-file#flamegraphs), which rack-mini-profiler and [stackprof](https://github.com/tmm1/stackprof) generate.
...
![Flamegraph with a call to `plain_text` taking 187.49ms](https://campsite.imgix.net/o/cl3gijjgd001/p/0de38489-7077-4a0b-8d4c-11487fe03c86.png?auto=compress%2Cformat&dpr=2&q=80&w=800)
...
![Flamegraph with many long calls to `Nokogiri::XML::Node#matches?](https://campsite.imgix.net/o/cl3gijjgd001/p/2f7b7b5d-37fe-42b1-b2d7-90f584a9f473.png?auto=compress%2Cformat&dpr=2&q=80&w=800)
...
![Flamegraph with a call to `plain_text` taking 0.96ms](https://campsite.imgix.net/o/cl3gijjgd001/p/18528d6c-91f7-4f08-bd1a-fa4c3f4a35f5.png?auto=compress%2Cformat&dpr=2&q=80&w=800)

## Less work, faster code

Looking at [the source](https://github.com/sparklemotion/nokogiri/blob/e05b9949b794ca94b37a90f2fb2555d99a37daa5/lib/nokogiri/xml/node.rb#L1088-L1092), when you call `Nokogiri::XML::Node#matches?`, Nokogiri…
